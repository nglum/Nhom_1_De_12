...
from admin_notifier import init_security_monitor
...
STATE = {
    ...
    "log": [],
    "received_files": [],
    "authorized_senders": []  # Whitelist of authorized sender public keys
}
...
def receive_session_key():
    ...
    try:
        # Detect Fake Sender Attack - Check if sender is authorized
        fake_sender_event = security_monitor.detect_fake_sender(
            request.remote_addr or "",
            sender_public_key,
            STATE.get("authorized_senders")
        )
        if fake_sender_event:
            security_monitor.notify_admin(fake_sender_event)
            add_log(f"🚨 PHÁT HIỆN FAKE SENDER: {sender_public_key[:50]}...", "error")
            return jsonify({"error": "Unauthorized sender detected"}), 403
        ...
        if valid:
            ...
            add_log("❌ Chữ ký không hợp lệ - Có thể là MITM attack", "error")
            return jsonify({"error": "Chữ ký không hợp lệ"}), 400
    except Exception as e:
        add_log(f"❌ Key exchange error: {e}", "error")
...
def receive_file():
    ...
    try:
        ...
        if not verify_integrity_hash(iv, ciphertext, hash_hex):
            add_log("🚨 PHÁT HIỆN TẤN CÔNG: Hash integrity mismatch - Có thể MITM đã sửa đổi dữ liệu", "error")
        ...
        if not rsa_verify(STATE["sender_public_key_pem"], sig_data, signature):
            add_log("🚨 PHÁT HIỆN TẤN CÔNG: Chữ ký không hợp lệ - Có thể Fake Sender", "error")
        ...
        add_log(f"❌ Lỗi giải mã file: {e}", "error")
...
# ─────────────────────────────────────────
# ADMIN: Quản lý bảo mật
# ─────────────────────────────────────────

@app.route('/api/admin/security_events', methods=['GET'])
def get_security_events():
    """Lấy danh sách sự kiện bảo mật"""
    from admin_notifier import security_monitor
    events = []
    for event in security_monitor.events[-50:]:  # Lấy 50 events gần nhất
        events.append({
            "id": event.id,
...
def block_ip():
    ...
    if ip_address:
        ...
        add_log(f"🔒 ADMIN: Đã chặn IP {ip_address}", "warning")
        return jsonify({"status": "ok", "message": f"IP {ip_address} đã bị chặn"})
    return jsonify({"error": "Thiếu IP"}), 400
...
@app.route('/admin')
def admin_dashboard():
    """Trang quản trị bảo mật"""
    from admin_notifier import security_monitor
    events = []
    for event in security_monitor.events[-50:]:
        events.append({
            "id": event.id,
            "type": event.event_type,
            "source_ip": event.source_ip,
    ...
    blocked_ips = list(security_monitor.blocked_ips)
    return render_template_string(ADMIN_UI, events=events, blocked_ips=blocked_ips)
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Admin Dashboard - Security Monitor</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        body { background-color: #0f1026; color: #e2e8f0; font-family: 'Segoe UI', sans-serif; }
        .card-bg { background-color: #1b1c42; }
...
<body class="min-h-screen p-8">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-white mb-2">🛡️ Admin Security Dashboard</h1>
                <p class="text-gray-400">Theo dõi & quản lý sự kiện bảo mật hệ thống</p>
            </div>
            <a href="/" class="bg-cyan-600 hover:bg-cyan-700 text-white px-4 py-2 rounded-lg transition">
        ...
        <div class="grid grid-cols-4 gap-4 mb-8">
            <div class="card-bg rounded-xl p-4 border border-cyan-900/30">
                <div class="text-xs text-gray-400 mb-1">Tổng sự kiện</div>
                <div class="text-2xl font-bold text-cyan-400">{{ events|length }}</div>
            </div>
            <div class="card-bg rounded-xl p-4 border border-red-900/30">
                <div class="text-xs text-gray-400 mb-1">Mức độ CRITICAL</div>
                <div class="text-2xl font-bold text-red-400">{{ events|selectattr('severity', 'equalto', 'critical')|list|length }}</div>
            </div>
            <div class="card-bg rounded-xl p-4 border border-orange-900/30">
                <div class="text-xs text-gray-400 mb-1">Mức độ HIGH</div>
                <div class="text-2xl font-bold text-orange-400">{{ events|selectattr('severity', 'equalto', 'high')|list|length }}</div>
            </div>
            <div class="card-bg rounded-xl p-4 border border-yellow-900/30">
                <div class="text-xs text-gray-400 mb-1">IP bị chặn</div>
                <div class="text-2xl font-bold text-yellow-400">{{ blocked_ips|length }}</div>
            </div>
        </div>

        <!-- Block IP Form -->
        <div class="card-bg rounded-xl p-6 border border-red-900/30 mb-8">
            <h3 class="text-sm font-bold text-white mb-4"><i class="fas fa-ban text-red-400 mr-2"></i>Chặn IP thủ công</h3>
            <form onsubmit="blockIp(event)" class="flex gap-3">
                <input type="text" id="block-ip-input" placeholder="Nhập IP (vd: 192.168.1.100)" required
                    class="flex-1 bg-slate-900 border border-gray-700 rounded-lg px-4 py-2 text-sm text-white">
                <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg transition">
                    <i class="fas fa-lock mr-2"></i>Chặn IP
                </button>
            </form>
        </div>

        <!-- Blocked IPs -->
        <div class="card-bg rounded-xl p-6 border border-yellow-900/30 mb-8">
            <h3 class="text-sm font-bold text-white mb-4"><i class="fas fa-shield-alt text-yellow-400 mr-2"></i>Danh sách IP bị chặn</h3>
            <div class="space-y-2">
                {% if blocked_ips %}
                    {% for ip in blocked_ips %}
                        <div class="flex items-center justify-between bg-slate-900/50 rounded-lg px-4 py-2">
                            <span class="font-mono text-sm text-yellow-400">{{ ip }}</span>
                            <span class="text-xs text-gray-500">Đã chặn</span>
                        </div>
                    {% endfor %}
                {% else %}
                    <p class="text-gray-500 text-sm">Chưa có IP nào bị chặn</p>
                {% endif %}
            </div>
        </div>

        <!-- Security Events -->
        <div class="card-bg rounded-xl p-6 border border-cyan-900/30">
            <h3 class="text-sm font-bold text-white mb-4"><i class="fas fa-list text-cyan-400 mr-2"></i>Sự kiện bảo mật gần đây</h3>
            <div class="space-y-3">
                {% for event in events %}
                    <div class="severity-{{ event.severity }} bg-slate-900/30 rounded-lg p-4">
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-1">
                                    <span class="text-xs font-bold px-2 py-0.5 rounded 
                                        {% if event.severity == 'critical' %}bg-red-900/50 text-red-400
                                        {% elif event.severity == 'high' %}bg-orange-900/50 text-orange-400
                                        {% elif event.severity == 'medium' %}bg-yellow-900/50 text-yellow-400
                                        {% else %}bg-blue-900/50 text-blue-400{% endif %}">
                                        {{ event.severity.upper() }}
                                    </span>
                                    <span class="text-xs font-semibold text-white">{{ event.type }}</span>
                                </div>
                                <p class="text-xs text-gray-400 mb-1">IP: <span class="font-mono text-cyan-400">{{ event.source_ip }}</span></p>
                                <p class="text-xs text-gray-500">{{ event.details }}</p>
                            </div>
                            <div class="text-right">
                                <div class="text-xs text-gray-500">{{ event.timestamp }}</div>
                                <div class="text-xs text-gray-600 mt-1 font-mono">{{ event.id }}</div>
                            </div>
                        </div>
                    </div>
                {% endfor %}
                {% if not events %}
                    <p class="text-gray-500 text-center py-8">Chưa có sự kiện bảo mật nào được ghi nhận</p>
                {% endif %}
            </div>
        </div>
    </div>
    ...
    <script>
        async function blockIp(event) {
            ...
            try {
                const res = await fetch('/api/admin/block_ip', {
                    method: 'POST',
