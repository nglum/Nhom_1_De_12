...
# Ensure the script's directory is in the module search path
...
# Import security monitor for hacker detection
from admin_notifier import security_monitor

# Known authorized receivers (can be configured)
AUTHORIZED_RECEIVERS = set()  # Populated dynamically or from config
...
def handshake():
    try:
        # Detect MITM - Check if receiver response is suspicious
        receiver_ip = get_receiver()
        
        t_start = time.perf_counter()
        resp = requests.post(f"{receiver_ip}/api/handshake",
                             json={"msg": "Hello!"},
                             timeout=10)
        ...
        # Suspicious response
        add_log(f"⚠️ Phản hồi handshake bất thường từ {receiver_ip}", "warning")
        return jsonify({"error": "Handshake thất bại"}), 400
    except Exception as e:
        # Detect potential MITM/DoS from connection errors
        mitm_event = security_monitor.detect_mitm(
            request.remote_addr or "",
            str(time.time()).encode()
        )
        if mitm_event:
            security_monitor.notify_admin(mitm_event)
            add_log(f"🚨 PHÁT HIỆN TẤN CÔNG MITM: {e}", "error")
...
def key_exchange():
    ...
    try:
        ...
        t_start = time.perf_counter()
        receiver_ip = get_receiver()
        resp = requests.get(f"{receiver_ip}/api/get_public_key", timeout=10)
        ...
        # Detect Fake Receiver - Check if receiver public key is suspicious
        if len(receiver_pub) < 100:
            add_log(f"🚨 PHÁT HIỆN FAKE RECEIVER: Public key quá ngắn ({len(receiver_pub)} chars)", "error")
            return jsonify({"error": "Suspicious receiver public key"}), 403
        ...
        # Gửi cho Receiver
        t_start = time.perf_counter()
        resp = requests.post(f"{receiver_ip}/api/receive_session_key", json={
        ...
        if result.get("status") == "ok":
            ...
            error_msg = result.get("error", "Thất bại")
            # Check if receiver detected an attack
            if "Unauthorized" in error_msg or "fake" in error_msg.lower():
                add_log(f"🚨 RECEIVER PHÁT HIỆN TẤN CÔNG: {error_msg}", "error")
...
def send_file_api():
    ...
    try:
        # Detect potential attacks before processing
        mitm_event = security_monitor.detect_mitm(
            request.remote_addr or "",
            STATE.get("session_key", b"")
        )
        if mitm_event:
            security_monitor.notify_admin(mitm_event)
            add_log("🚨 PHÁT HIỆN TẤN CÔNG MITM TRƯỚC KHI GỬI FILE!", "error")
        ...
        if result.get("status") == "ACK":
            ...
            add_log(f"❌ NACK: {error_msg}", "error")
            
            # Check if receiver detected an attack
            if "không hợp lệ" in error_msg or "không khớp" in error_msg:
                add_log("🚨 RECEIVER PHÁT HIỆN TẤN CÔNG - File bị từ chối!", "error")
            
            return jsonify({"status": "NACK", "error": error_msg}), 400
